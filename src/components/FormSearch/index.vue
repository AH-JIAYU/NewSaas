    ...
    <SearchBar :show-toggle="false">
      <template #default="{ fold, toggle }">
        <el-form :model="props.model" size="default" label-width="100px" inline-message inline class="search-form">
          <template v-for="(item, index) in formSearchList" :key="item.modelName">
            <el-form-item label="" v-if="item.show" v-show="index > 2 ? !fold : true">
          ...
          <ElFormItem>
            ...
            <ElButton class="p-2" type="primary" @click="onSort" v-if="formSearchList?.length >= 6">
              <template #icon>
                <svg width="13" height="12" viewBox="0 0 13 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <defs>
                    <clipPath id="clip0_402_17380">
                      <rect width="12" height="12" fill="white" transform="translate(0.742188)" />
                    </clipPath>
                  </defs>
                </svg>
              </template>
            ...
            <ElButton class="p-2" link @click="toggle" :disabled="showLength <= 3">
              <template #icon>
