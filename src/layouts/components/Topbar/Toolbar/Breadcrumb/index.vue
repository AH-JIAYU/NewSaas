...
const breadcrumbList = computed(() => {
  ...
  if (
    route.fullPath !== settingsStore.settings.home.fullPath &&
    ...
    const index = menuStore.allMenus.findIndex((item) =>
      item.children.some(
        (r) =>
          route.fullPath.indexOf(`${r.path}/`) === 0 ||
          route.fullPath === r.path,
      ),
    );
    menuStore.allMenus[index]?.meta &&
      breadcrumbList.push({
        path: "",
        title: generateI18nTitle(
          menuStore.allMenus[index].meta?.i18n,
          menuStore.allMenus[index].meta?.title,
        ),
      });
  }
  if (route.meta.breadcrumbNeste) {
    ...
    const findItem = settingsStore.customTitleList.find(
...
<template>
  <Breadcrumb
    ...
    :class="{
      [`breadcrumb-${settingsStore.settings.breadcrumb.style}`]:
        settingsStore.settings.breadcrumb.style !== '',
    }"
  >
