...
<template>
  <PageMain>
    <el-tabs v-model="activeName" class="demo-tabs">
            ...
            <el-table v-loading="listLoading" :border="border" :data="list" :size="lineHeight" :stripe="stripe"
              ...
              <ElTableColumn v-if="checkList.includes('customerStatus')" align="left" show-overflow-tooltip
                ...
                <template #default="{ row }">
                  <ElSwitch v-model="row.customerStatus" inline-prompt :inactive-value="1" :active-value="2"
                    active-text="启用" inactive-text="禁用" @change="changeState($event, 1, row.tenantCustomerId)" />
                </template>
              </ElTableColumn>
              <el-table-column v-if="checkList.includes('customerAccord')" align="left" prop="customerAccord"
                ...
                <template #default="{ row }">
                  <div class="flex-c tableBig">
                    ...
                    <copy :content="row.customerAccord" :class="{
      rowCopy: 'rowCopy',
                ...
                <template #default="{ row }">
                  <div class="flex-c tableBig">
                    ...
                    <SvgIcon @click="quickEdit(row, 'customerShortName')"
                      :class="{ edit: 'edit', current: row.tenantCustomerId === current }" name="i-ep:edit"
                      color="#409eff" />
                  </div>
                </template>
              </el-table-column>
              <el-table-column v-if="checkList.includes('tenantCustomerId')" align="left" prop="tenantCustomerId"
                ...
                <template #default="{ row }">
                  <div class="copyId tableSmall">
                    ...
                    <copy :content="row.tenantCustomerId" :class="{
      rowCopy: 'rowCopy',
      current: row.tenantCustomerId === current,
    }" />
                    <!-- <copy /> -->
              ...
              <el-table-column v-if="checkList.includes('projectNumber')" align="left" prop="turnover"
                show-overflow-tooltip label="关联项目数">
                <template #default="{ row }">
              ...
              <ElTableColumn v-if="checkList.includes('riskControl')" align="left" show-overflow-tooltip
                prop="riskControl" label="风险控制">
                <template #default="{ row }">
                  <ElSwitch v-model="row.riskControl" inline-prompt :inactive-value="1" :active-value="2"
                    active-text="启用" inactive-text="禁用" @change="changeState($event, 3, row.tenantCustomerId)" />
                </template>
              </ElTableColumn>
              ...
              <el-table-column v-if="checkList.includes('rateAudit')" align="left" prop="rateAudit"
                show-overflow-tooltip label="审核率Min值"><template #default="{ row }">
      ...
      <el-tab-pane label="合作商户" name="second">
        <cooperation />
      </el-tab-pane>
    </el-tabs>
  </PageMain>
