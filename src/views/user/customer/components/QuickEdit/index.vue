      ...
      <el-form ref="formRef" :model="data.formData" :rules="data.rules" label-width="100" label-position="right">
          ...
          <template v-if="data.type === 'chargeName'">
            <el-form-item label="负责人" prop="chargeId">
                  <el-select v-model="data.formData.chargeId" value-key="" placeholder="请选择负责人" clearable filterable>
                    <el-option v-for="item in data.staffList" :key="item.id" :label="item.name" :value="item.id" />
                  </el-select>
                </el-form-item>
        </template>
      </el-form>
