    ...
    <PageMain>
      ...
      <el-table ref="tableSortRef" v-loading="listLoading" style="margin-top: 10px" row-key="id" :data="DataList"
        ...
        <el-table-column v-if="checkList.includes('projectIdentificationOrClientName')" show-overflow-tooltip
          prop="projectIdentificationOrClientName" align="center" label="客户简称/标识" />

        <el-table-column v-if="checkList.includes('participation')" show-overflow-tooltip align="center"
          label="参与/完成/配额/限量">
        ...
        <el-table-column v-if="checkList.includes('doMoneyPrice')" show-overflow-tooltip align="center" label="原价">
          <template #default="{ row }">
            <el-text v-if="countryType === 1" class="mx-1">${{ row.doMoneyPrice }}</el-text>
            <el-text v-if="countryType === 2" class="mx-1">￥{{ row.doMoneyPrice }}</el-text>
          ...
          <template #default="{ row }">
            <template v-if="row.countryNameList.length > 4">
              <el-tooltip
                ...
                >
                  <el-link type="primary"><el-tag type="danger">{{  row.countryNameList.length
                }}</el-tag></el-link>
                </el-tooltip>
