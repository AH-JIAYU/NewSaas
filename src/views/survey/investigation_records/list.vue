...
function currentChange(page = 1) {
  onCurrentChange(page).then(() => {
    surveyStatus.value = []
    queryForm.viceStatus = []
    queryForm.surveyStatus.map((item: any) => {
        if (typeof item === 'string') {
          queryForm.viceStatus.push(+item);
        }else {
          surveyStatus.value.push(item);
        }
      })
    queryForm.page = page
    fetchData()
...
async function fetchData() {
  try {
    ...
    params.surveyStatus = surveyStatus.value
        ...
        <ElTableColumn v-if="checkList.includes('surveyStatus')" align="left" show-overflow-tooltip prop="" label="调查状态"
          ...
          <template #default="{ row }">
            ...
            <el-tag effect="dark" style="background-color: #74868b; border: none"
              v-if="row.surveyStatus === 1 && row.viceStatus === 1" class="mx-1" type="success">待审核</el-tag>
