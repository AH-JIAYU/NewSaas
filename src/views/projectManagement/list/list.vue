...
<template>
  ...
  >
    <PageMain class="hide-drawer-header">
      <FormSearch
        :form-search-list="formSearchList" :form-search-name="formSearchName" :model="search"
      ...
      <el-row :gutter="24">
        <FormLeftPanel>
          ...
          <el-button v-auth="'/list-get-addProject'" type="primary" size="default" @click="addProject">
            新增项目
          </el-button>
          <el-button v-auth="'/list-get-addProjectDispatch'" type="primary" size="default" @click="dispatch">
            调度
          </el-button>
          <el-checkbox
            v-model="checked1" style="margin-left: 12px"   :label="t('project.QueryDepartmentList')" size="large"
            @change="handleChange"
          />
        </FormLeftPanel>
        <FormRightPanel>
      ...
      <el-table
        ref="tableSortRef" v-loading="listLoading" style="margin-top: 0.625rem" row-key="projectId" :data="list"
      ...
      >
        ...
        <el-table-column
          v-if="checkList.includes('isOnline')" show-overflow-tooltip prop="isOnline" align="left"
          width="84"  :label="t('project.status')"
        >
          <template #default="{ row }">
            <ElSwitch
              v-model="row.isOnline" inline-prompt   :active-text="t('project.Online')"
              :inactive-text="t('project.Offline')"
              :active-value="1" :inactive-value="2" @change="changeStatus(row, $event)"
            />
          </template>
        </el-table-column>
        <el-table-column v-if="checkList.includes('projectId')" align="left"  :label="t('project.project')" width="180">
          <template #default="{ row }">
            <p>
              <span class="tableBig">
                ...
                <el-tag
                  v-else-if="row.projectType === 2" type="warning" effect="dark"
        ...
        <el-table-column
          v-if="checkList.includes('name')" show-overflow-tooltip prop="name" align="left" :label="t('project.name')"
          width="200"
        >
          <template #default="{ row }">
            <div class="flex-c">
              ...
              <SvgIcon
                v-if="row.projectType !== 2" v-auth="'/list-update-updateProject'"
        ...
        <el-table-column
          v-if="checkList.includes('countryIdList')" show-overflow-tooltip prop="countryIdList"
          align="left"    :label="t('project.area')" width="80"
        >
          <template #default="{ row }">
            <template v-if="row.countryIdList">
              ...
              <template v-else-if="comCountryId(row.countryIdList).length > 1">
                <el-tooltip
                  class="box-item" effect="dark" :content="comCountryId(row.countryIdList).join(',')"
                  placement="top"
                >
        ...
        <el-table-column v-if="checkList.includes('PCNL')" align="left"   :label="t('project.parameter')" width="200">
          <template #default="{ row }">
            <div class="flex-c">
              <el-tooltip class="box-item" effect="dark" :content="row.participation" placement="top-start">
                <template #content>
                  {{ t("project.participateIn2") }}: {{ row.participation ? row.participation : "-" }}  {{ t("project.complete2") }}:
                  {{ row.complete ? row.complete : "-" }}  {{ t("project.quota2") }}:
                  {{ row.num ? row.num : "-" }} {{ t("project.limited2") }}:
                  {{ row.limitedQuantity ? row.limitedQuantity : "-" }}
                </template>
                <div class="oneLine parameter" style="width: calc(100% - 1.25rem)">
                  <el-text style="color: rgb(251, 104, 104)" class="oneLine" type="danger">
                    {{ t("project.participateIn2") }}: {{ row.participation || 0
                    }}
                  </el-text>
                  <el-text style="color: rgb(3, 194, 57)" class="oneLine" type="success">
                    {{ t("project.complete2") }}: {{ row.complete || 0
                    }}
                  </el-text>
                  <el-text style="color: rgb(255, 172, 84)" class="oneLine" type="warning">
                    {{ t("project.quota2") }}: {{ row.num || 0
                    }}
                  </el-text>
                  <el-text style="color: rgb(170, 170, 170)" class="oneLine" type="info">
                    {{ t("project.limited2") }}: {{ row.limitedQuantity
                      || "-"
                    }}
                  </el-text>
                </div>
        ...
        <el-table-column
          v-if="checkList.includes('doMoneyPrice')" show-overflow-tooltip align="left" :label="t('project.projectPrice')"
          width="110"
        >
          <template #default="{ row }">
            <div class="flex-c">
              <div v-if="row.originalPrice" class="oneLine fontC-System" style="width: calc(100% - 1.25rem)">
                <svg
                  v-if="row.beCurrencyType === 'USD'" xmlns="http://www.w3.org/2000/svg" width="7" height="12"
                ...
                <svg
                  v-if="row.beCurrencyType === 'CNY'" xmlns="http://www.w3.org/2000/svg" width="9" height="10"
        ...
        <el-table-column
          v-if="checkList.includes('ir')" show-overflow-tooltip prop="ir" align="left" label="IR/NIR"
          width="80"
        >
          <template #default="{ row }">
            <div class="flex-c">
              ...
              <SvgIcon
                v-if="row.projectType !== 2" :class="{ edit: 'edit', current: row.projectId === current }"
                name="i-ep:edit" color="#409eff" @click="quickEdit(row, 'ir')"
              />
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column v-if="checkList.includes('allocationType')" align="left" label="分配" width="120">
          <template #default="{ row }">
            ...
            <div v-if="row.allocationStatus == 2" class="flex-c" style="cursor: pointer">
              <div style="width: calc(100% - 1.25rem)" class="parameter1" @click="viewAllocations(row)">
                <el-tag v-if="row.allocationType?.includes(2)" type="danger" class="tag-with-image oneLine">
                  <img
                ...
                <el-tag v-if="row.allocationType?.includes(3)" type="warning" class="tag-with-image oneLine">
                  <img
                    src="@/assets/images/nei.png" style="
                      width: 0.9375rem;
                      height: 0.9375rem;
                      margin-right: 0.25rem;
                    "
                ...
                <el-tag v-if="row.allocationType?.includes(4)" type="primary" class="tag-with-image oneLine">
                  <img
                    src="@/assets/images/he.png" style="
                      width: 0.9375rem;
                      height: 0.9375rem;
                      margin-right: 0.25rem;
                    "
        ...
        <el-table-column
          v-if="checkList.includes('remark')" show-overflow-tooltip prop="remark" align="left"
          width="120"  :label="t('project.remark')"
        >
          <template #default="{ row }">
            <div class="flex-c">
              <div class="oneLine fontC-System" style="width: calc(100% - 1.25rem)">
              ...
              <SvgIcon
                v-if="row.projectType !== 2" :class="{ edit: 'edit', current: row.projectId === current }"
                name="i-ep:edit" color="#409eff" @click="quickEdit(row, 'remark')"
              />
            </div>
          </template>
        </el-table-column>
        <el-table-column
          v-if="checkList.includes('clientName')" prop="clientName" show-overflow-tooltip align="left"
          :label="t('project.client')" width="140"
        >
          <template #default="{ row }">
            <div class="flex-c">
              <div class="oneLine" style="width: calc(100% - 1.25rem)">
        ...
        <el-table-column
          v-if="checkList.includes('create')" prop="createTime" align="left" width="120" :label="t('project.create')"
          show-overflow-tooltip
        >
          <template #default="{ row }">
            <div>
        ...
        <el-table-column align="left" fixed="right"  :label="t('project.operate')" width="230">
          <template #default="{ row }">
            <el-button
              v-if="row.allocationStatus === 1" v-auth="'/list-get-addProjectAllocation'" plain :disabled="row.isOnline === 2"
              type="primary" size="small" @click="distribution(row)"
            >
            {{ t("project.distribution") }}
            </el-button>
            <el-button
              v-else v-auth="'/list-get-addProjectAllocation'" plain type="primary" :disabled="row.isOnline === 2" size="small"
              @click="reassign(row)"
            >
            ...
            <el-button
              v-auth="'/list-update-updateProject'" type="warning" plain size="small"
              @click="projectEdit(row)"
            >
            {{ t("project.editor") }}
            </el-button>
            <el-button v-auth="'/list-get-getProject'" type="danger" size="small" plain @click="projectDetails(row)">
              {{ t("project.details") }}
            </el-button>
          </template>
        </el-table-column>
