...
import wangEditor from '@/components/UEditor/wangEditor.vue'
// 下载
...
const projectManagementListStore = useProjectManagementListStore()
...
const fold = ref(!props.tabIndex) // 折叠 描述配额
...
// 自定义校验邮箱
function validateUrlRegistered(rule: any, value: any, callback: any) {
  // 网址格式
  const regExpUrl: any
    = /^(((ht|f)tps?):\/\/)?([^!@#$%^&*?.\s-]([^!@#$%^&*?.\s]{0,63}[^!@#$%^&*?.\s])?\.)+[a-z]{2,6}\/?/
  if (!regExpUrl.test(localToptTab.value.uidUrl)) {
    callback(new Error(t('projectTopTabs.validUrl')))
  }
  if (!localToptTab.value.uidUrl.includes('[uid]')) {
    callback(new Error(t('projectTopTabs.incorrectFormat')))
  }
  callback()
}
// 自定义校验发布时间
function validateReleaseTime(rule: any, value: any, callback: any) {
  if (new Date(localToptTab.value.releaseTime).getTime() < Date.now()) {
    callback(new Error(t('projectTopTabs.releaseTime')))
  }
  callback()
}
// 校验
const rules = reactive<any>({
  name: [
    {
      required: true,
      message: t('projectTopTabs.projectName'),
      trigger: 'blur',
    },
    { min: 2, max: 50, message: t('projectTopTabs.2to50'), trigger: 'blur' },
  ],
  projectIdentification: [
    {
      max: 100,
      required: true,
      message: t('projectTopTabs.projectIdentification'),
      trigger: 'blur',
    },
    { min: 2, max: 100, message: t('projectTopTabs.2to100'), trigger: 'blur' },
  ],
  countryIdList: [
    {
      required: true,
      message: t('projectTopTabs.area'),
      trigger: 'change',
    },
  ],
  clientId: [
    {
      required: true,
      message: t('projectTopTabs.customer'),
      trigger: 'change',
    },
  ],
  currencyType: [
    {
      required: true,
      message: t('projectTopTabs.currency'),
      trigger: 'change',
    },
  ],
  uidUrl: [
    { required: true, message: t('projectTopTabs.url'), trigger: 'blur' },
  ...
  doMoneyPrice: [
    {
      required: true,
      message: t('projectTopTabs.projectPrice'),
      trigger: 'blur',
    },
    {
      pattern: /^(?!0(\.0+)?$)(\d+(\.\d{1,2})?)$/,
      message: t('projectTopTabs.bt0'),
      trigger: 'blur',
    },
  ],
  num: [
    { required: true, message: t('projectTopTabs.quota'), trigger: 'blur' },
    {
      pattern: /^[0-9]+$/,
      message: t('projectTopTabs.positiveInteger'),
      trigger: 'blur',
    },
  ],
  minimumDuration: [
    {
      required: true,
      message: t('projectTopTabs.minimumLength'),
      trigger: 'blur',
    },
    {
      pattern: /^(?!0(\.0+)?$)(\d+(\.\d{1,2})?)$/,
  ...
  ir: [
    { required: true, message: t('projectTopTabs.ir'), trigger: 'blur' },
    {
      pattern: /^(100|[1-9]?[0-9])$/,
      message: t('projectTopTabs.0to100'),
      trigger: 'blur',
    },
  ],
  exchangeRate: [
    {
      required: true,
      message: t('projectTopTabs.exchangeRate'),
      trigger: 'blur',
    },
    {
      pattern: /^(?!0(\.0+)?$)(\d+(\.\d{1,2})?)$/,
  ...
  sex: [
    { required: true, message: t('projectTopTabs.sex'), trigger: 'change' },
  ],
  startAge: [
    { required: true, message: t('projectTopTabs.age'), trigger: 'blur' },
    {
...
// 富文本框
function changeEditorValue(v: string) {
  localToptTab.value.richText = v
}
// 同步
...
// 是否是图片
function isItAPicture(file: any) {
  ...
  // 是否 非图片格式
  const isImage = !whiteList.includes(fileSuffix)
  return isImage
}
// 上传前"jpg",
...
// 回显图片
async function getUpLoad(file: any) {
  if (file.length) {
...
// 定时发布
function changeTimeReleases(val: any) {
  localToptTab.value.isOnline = val === 2 ? 2 : 1
}

// 切换问卷如果关 清空问卷list
function changeProfile(val: any) {
...
// 配置区域改变 重新获取题库目录
function changeConfigurationCountryId() {
...
// 清除数据
function clearData(judge?: boolean) {
...
// 切换tab 获取区域集合
async function changeTab(val: any, judge?: boolean) {
  if (
    ...
    formRef.value.validateField(['countryIdList'], async (valid: any) => {
      if (valid) {
        ...
        nextTick(() => {
          ...
          ElMessage.warning({ message: t('formRef.areaSelect'), center: true })
        })
      }
    })
  }
...
// 获取题库目录
async function getProjectCategoryList() {
  // 如果配置中的区域不存在就请求，反正不请
  if (
    localToptTab.value.data.configurationInformation.initialProblem.countryId
  ) {
...
// 根据目录id查询问题和答案表
async function getProjectProblemList(id: string | number, judge: boolean) {
  // 选中值
  if (id) {
    // 置空数据
    setTimeout(async () => {
...
function setAnswerValue(type: number, index: number) {
  // 选择的id "" | []
  const id: any
...
// 编辑时 回显配置信息
async function showProjectQuotaInfoList() {
 ...
 */
function customModel(id: any, index: any) {
  return {
    get() {
...
// 提交汇率
async function exchangeRateSubmit() {
...
// #endregion
// 切换币种
async function handleSelectChange(val: any) {
  const params = {
    beforeCurrency: '',
    oldCurrency: val,
    exchangeRate: '',
...
<template>
  ...
  >
    <el-tabs v-model="activeName" @tab-change="changeTab">
      <el-tab-pane :label="t('newProject.basicSettings')" name="basicSettings">
        <el-card body-style="">
          ...
          <el-row :gutter="20">
            ...
            <el-col v-if="localToptTab.projectType !== 2" :span="6">
              <el-form-item
                :label="t('newProject.projectIdentification')"
              ...
              <el-form-item :label="t('newProject.clientId')" prop="clientId">
                ...
                >
                  ...
                  >
                    ...
                    <div class="i-ic:round-plus h-1.3em w-1.3em" />
                    <!-- <SvgIcon
            ...
            <el-col :span="6">
              <el-form-item
                :label="t('newProject.countryIdList')"
                prop="countryIdList"
              >
                <ElSelect
            ...
            <el-col :span="6">
              ...
              <el-form-item :label="t('newProject.exchangeRate')" prop="">
                <el-text v-if="currencyTypeRes !== null">
                  1{{ t("newProject.dollor") }} = {{ acquiesceExchangeRate
                  }}{{ t("newProject.rmb") }}
                </el-text>
              </el-form-item>
            </el-col>
          ...
          <el-row :gutter="20">
            <el-col :span="6">
              <el-form-item
                :label="t('newProject.projectPrice')"
                prop="doMoneyPrice"
              >
                <el-input
        ...
        <el-card v-if="!localToptTab.required">
          ...
          <el-row :gutter="20">
            <el-col :span="5">
              ...
              >
                ...
                >
                  <el-radio-button
                    :value="1"
                    :label="t('newProject.unlimited')"
                  />
                  <el-radio-button
                    :value="2"
                    :label="t('newProject.male')"
                  />
                  <el-radio-button
                    :value="3"
                    :label="t('newProject.female')"
                  />
                </el-radio-group>
              </el-form-item>
            </el-col>
            ...
            <el-col :span="6">
              <el-form-item
                :label="t('newProject.operatingSystem')"
                class="flex"
              >
                <el-select
          ...
          <el-row :gutter="20">
            <el-col :span="5">
              <el-form-item
                :label="t('newProject.scheduledRelease')"
                class="flex"
              >
                <el-switch
            ...
            <el-col :span="5">
              <el-form-item
                :label="t('newProject.preRuestionnaire')"
                class="flex"
              >
                <el-switch
          ...
          <el-row :gutter="20">
            <!-- 定时发布开显示时间，关隐藏 -->
            <el-col v-if="localToptTab.isTimeReleases === 2" :span="5">
              <el-form-item
                :label="t('newProject.releaseTime')"
                prop="releaseTime"
              >
                <el-date-picker
            ...
            <el-form-item :label="t('newProject.uploadIF')">
              ...
              >
                ...
                <div v-for="item in fileList" :key="item.name">
                  <div class="i-f7:doc-text h-1rem w-1rem" />
                  <el-link
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item :label="t('newProject.projectDescribe')">
                  ...
                  <wangEditor
                    style="width: 100%"
                    :value-html="localToptTab.richText"
                    @change-editor-value="changeEditorValue"
                  />
                </el-form-item>
              </el-col>
            </el-row>
      ...
      <el-tab-pane
        v-if="
          localToptTab.isProfile === 1
            ...
            && localToptTab.projectType !== 2
        "
        :label="t('projectTopTabs.configurationInformation')"
        name="configurationInformation"
      >
        <el-card v-if="localToptTab.data">
          <template #header>
            <div class="card-header">
              {{ t("projectTopTabs.configurationInformation") }}
            </div>
          </template>
          <el-row :gutter="20">
            <el-col :span="6">
              <el-form-item :label="t('projectTopTabs.areaSelect')">
                <el-select
                  v-model="
            ...
            <el-col
              ...
              :span="6"
            >
              <el-form-item :label="t('projectTopTabs.questionnaireName')">
                <el-select
                  v-model="
                    localToptTab.data.configurationInformation.initialProblem
                      .projectProblemCategoryId
          ...
          >
            ...
            >
              <el-col :span="20">
                {{ t("projectTopTabs.question") }}：{{ item.question }}
              </el-col>
              <el-col :span="20">
                <el-form-item>
                  <!-- 1输入框 -->
                  ...
                  <el-checkbox-group
                    v-else
                    :model-value="customModel(item.id, index).get()"
                    @update:model-value="customModel(item.id, index).set($event)"
      ...
      >
        <el-card>
          <template #header>
            <div class="card-header">
              {{ t("newProject.safetyInformation") }}
            </div>
          </template>
          <el-row :gutter="20">
            <el-col :span="6">
              <el-form-item>
                <template #label>
                  <div>
                    {{ t("newProject.hourly")
                    }}<span class="red">{{ t("newProject.inflow") }}</span>{{ t("newProject.num") }}
                  </div>
                </template>
                <el-input-number
            ...
            <el-col :span="6">
              <el-form-item>
                <template #label>
                  <div>
                    {{ t("newProject.hourly")
                    }}<span class="blue">{{ t("newProject.completion") }}</span>{{ t("newProject.num") }}
                  </div>
                </template>
                <el-input-number
            ...
            <el-col :span="4" class="flex">
              <el-form-item label="" class="m-0 flex">
                <template #label>
                  <div>
                    {{ t("newProject.allowRepeat")
    ...
    <Edit
      ...
      :country-id="data.editProps.countryId"
