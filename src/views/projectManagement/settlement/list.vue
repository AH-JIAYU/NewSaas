  ...
  <div :class="{
    ...
    <PageMain>
      ...
      <el-table ref="tableSortRef" v-loading="listLoading" style="margin-top: 10px" row-key="id" :data="list"
        :border="border" :size="lineHeight" :stripe="stripe" highlight-current-row @selection-change="setSelectRows"
        @current-change="handleCurrentChange">
        <el-table-column align="left" type="selection" />
        <el-table-column v-if="checkList.includes('projectId')" prop="projectId" align="left" label="项目">
    ...
    current: row.projectId === current,
        ...
        <el-table-column v-if="checkList.includes('projectName')" show-overflow-tooltip prop="projectName" width="220"
          align="left" label="名称/标识">
          <template #default="{ row }">
            <div>
              <p class="oneLine tableBig">{{ row.projectName ? row.projectName : "-" }}</p>
              <div class="oneLine">
                {{
    row.customerName.split("/")[1] !== "null"
      ...
      : "-"
        ...
        <el-table-column v-if="checkList.includes('customerName')" show-overflow-tooltip prop="customerName" width="140"
          align="left" label="客户">
          <template #default="{ row }">
            <div class="oneLine">
        ...
        <el-table-column v-if="checkList.includes('projectAmount')" show-overflow-tooltip align="left" label="项目价"
          width="80">
          <template #default="{ row }">
            <div class="fontC-System">
              ...
              <CurrencyType v-if="!row.currencyType" />{{ row.projectAmount || 0 }}
              <!-- <CurrencyType />{{ row.projectAmount || 0 }} -->
            </div>
          </template>
        </el-table-column>
        <el-table-column v-if="checkList.includes('countryId')" show-overflow-tooltip align="left" width="100"
          label="区域">
          <template #default="{ row }">
    ...
    ">
              ...
              <template v-else-if="comCountryId(row.countryId).length > 1">
                <el-tooltip class="box-item" effect="dark" :content="comCountryId(row.countryId).join(',')"
                  placement="top">
                  <el-link type="primary"><el-tag type="primary">×{{ comCountryId(row.countryId).length
                      }}</el-tag></el-link>
                </el-tooltip>
              </template>
              <template v-else>
    ...
    row.settlementDone ? row.settlementDone : 0
              ...
              <SvgIcon @click="quickEdit(row, 'systemDone')"
        ...
        <el-table-column v-if="checkList.includes('settlementPo')" show-overflow-tooltip prop="settlementPo"
          ...
          <template #default="{ row }">
            <div class="flex-c fontC-System">
              ...
              <SvgIcon @click="quickEdit(row, 'settlementPo')"
                :class="{ edit: 'edit', current: row.projectId === current }" name="i-ep:edit" color="#409eff" />
            </div>
          </template>
        </el-table-column>
        <el-table-column v-if="checkList.includes('status')" width="100" show-overflow-tooltip prop="status"
          align="left" label="状态">
          <template #default="{ row }">
            <div style="display: flex" class="oneLine">
              <el-text v-if="row.status === 1" class="mx-1 tableBig flex-c">
                <el-button style="margin-right: 5px;background-color:#FFAC54" size="small" class="p-1" type="warning"
                  @click="handleMoreStatus(row)">待审核</el-button>
                <!-- <el-text>{{ row.pendReviewTime }}</el-text> -->
              </el-text>
              <el-text v-if="row.status === 3" class="mx-1 tableBig flex-c">
                <el-button style="margin-right: 6px" size="small" class="p-1" type="success"
                  @click="handleMoreStatus(row)">已开票</el-button>
                <!-- <el-text>{{ row.invoicedOutTime }}</el-text> -->
              </el-text>
              <el-text v-if="row.status === 4" class="mx-1 tableBig flex-c">
                <el-button style="margin-right: 6px" size="small" class="p-1" type="info"
                  @click="handleMoreStatus(row)">已结算</el-button>
                <!-- <el-text>{{ row.settledTime }}</el-text> -->
              </el-text>
              <el-text v-if="row.status === 2" class="mx-1 tableBig flex-c">
                <el-button style="margin-right: 6px" size="small" class="p-1" type="primary"
                  @click="handleMoreStatus(row)">已审核</el-button>
                <!-- <el-text>{{ row.reviewTime }}</el-text> -->
              </el-text>
              <el-text v-if="row.status === 5" class="mx-1 tableBig flex-c">
                <el-button style="margin-right: 6px" size="small" class="p-1" type="danger"
                  @click="handleMoreStatus(row)">已冻结</el-button>
        ...
        <el-table-column v-if="checkList.includes('nodeTime')" show-overflow-tooltip align="left" label="时间"
          width="120">
          <template #default="{ row }">
        ...
        <el-table-column v-if="checkList.includes('remark')" show-overflow-tooltip prop="remark" align="left"
          ...
          <template #default="{ row }">
            <div class="flex-c fontC-System">
              ...
              <SvgIcon @click="quickEdit(row, 'remark')" :class="{ edit: 'edit', current: row.projectId === current }"
